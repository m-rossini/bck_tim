...
inputFiles=$1
	if [[ -z "${inputFiles}" || ! ( -r ${inputFiles} ) ]]; then
		echo "Usage: npack-loader.sh <input-file>"
		exit 2
	fi
...
${JAVA_HOME}/bin/java \
  ...
  -Dloader.layout=conf/megatim/megatim.properties \
  -Dlog4j.configuration=conf/megatim/log4j.xml \
...
${JAVA_HOME}/bin/java \
  ...
  org.apache.tools.ant.launch.Launcher \
  -Dloader.db=conf/loader.xml \
  -Dloader.layout=conf/megatim/megatim-services.properties \
  -Dlog4j.configuration=conf/megatim/log4j.xml \
  -file ${BCK_BASEDIR}/cmdline/bin/loader-run.xml
