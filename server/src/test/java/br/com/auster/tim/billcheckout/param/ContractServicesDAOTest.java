...
public class ContractServicesDAOTest extends TestCase {
	...
	public void testInitEnvironmentConnection(){
		...
		try {
			...
			conn = SQLConnectionManager.getInstance("bgh/guiding/guidingdb").getConnection();
			...
			conn = SQLConnectionManager.getInstance("bgh/guiding/testPool").getConnection();
			dbMetaData = conn.getMetaData();
			...
			conn = SQLConnectionManager.getInstance("bgh/guiding/testPool2").getConnection();
			dbMetaData = conn.getMetaData();
			assertEquals("jdbc:oracle:thin:@MCCOY:1521:DEVEL01", dbMetaData.getURL());
