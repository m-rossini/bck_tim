...
function listPackages() {
  ...
  echo "SELECT SHORT_DESC as \"Shdes\", DESCRIPTION as \"Nome do Pacote\" FROM QLF_PACKAGE WHERE CUSTOM_2 IS NOT NULL;" >> ${COMMANDFILE}
...
function addPackage() {
  ...
  echo "SET PAGESIZE 2000;" >> ${COMMANDFILE}
  echo "SET LINESIZE 2000;" >> ${COMMANDFILE}
  echo "UPDATE QLF_PACKAGE SET CUSTOM_2='T' WHERE SHORT_DESC = '${PACKAGE}';" >> ${COMMANDFILE}
...
function removePackage() {
  ...
  echo "SET PAGESIZE 2000;" >> ${COMMANDFILE}
  echo "SET LINESIZE 2000;" >> ${COMMANDFILE}
  echo "UPDATE QLF_PACKAGE SET CUSTOM_2=NULL WHERE SHORT_DESC = '${PACKAGE}';" >> ${COMMANDFILE}
  echo "COMMIT;" >> ${COMMANDFILE}
  echo "EXIT;" >> ${COMMANDFILE}
}
...
function displayUsage() {
	echo "Uso: pacote-infinity.sh <COMANDO> [<ID-DO-PACOTE>]"
	exit 1
}
