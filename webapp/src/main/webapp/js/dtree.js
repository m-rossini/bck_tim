...
// ----------------------------------------------------------------------------
	...
	this.preloadIcons = function()
	{
		if (this.hasLines)
		{
			...
			this.arrIcons[0].src = this.imgFolder + 'plus.png';
			this.arrIcons[1] = new Image();
			this.arrIcons[1].src = this.imgFolder + 'plusbottom.png';
			this.arrIcons[2] = new Image();
			this.arrIcons[2].src = this.imgFolder + 'minus.png';
			this.arrIcons[3] = new Image();
			this.arrIcons[3].src = this.imgFolder + 'minusbottom.png';
		}
		else
		{
			this.arrIcons[0] = new Image();
			this.arrIcons[0].src = this.imgFolder + 'nolines_plus.png';
			this.arrIcons[1] = new Image();
			this.arrIcons[1].src = this.imgFolder + 'nolines_plus.png';
			this.arrIcons[2] = new Image();
			this.arrIcons[2].src = this.imgFolder + 'nolines_minus.png';
			this.arrIcons[3] = new Image();
			this.arrIcons[3].src = this.imgFolder + 'nolines_minus.png';
		}
		this.arrIcons[4] = new Image();
		this.arrIcons[4].src = this.imgFolder + 'user.png';
		this.arrIcons[5] = new Image();
		this.arrIcons[5].src = this.imgFolder + 'user.png';
	}
			...
			if (this.arrNodes[n].pid == pNode)
			{
				...
				if (this.rootNode != cn.pid)
				{
					...
					for (r=0; r<this.arrRecursed.length; r++)
						this.strOutput += '<img src="' + this.imgFolder + ( (this.arrRecursed[r] == 1 && this.hasLines) ? 'line.png' : 'empty.gif' ) + '" alt="" />';
					...
					if (cn._hc)
					{
						...
						this.strOutput += ( (cn._io) ? ((cn._ls && this.hasLines) ? 'minusbottom.png' : 'minus.png') : ((cn._ls && this.hasLines) ? 'plusbottom.png' : 'plus.png' ) )
							+ '" alt="" /></a>';
					}
					else
						this.strOutput += '<img src="' + this.imgFolder + ( (this.hasLines) ? ((cn._ls) ? 'joinbottom.png' : 'join.png' ) : 'empty.gif') + '" alt="" />';
				}
				...
				this.strOutput += (cn.img) ? cn.img : ((this.rootNode == cn.pid) ? 'group' : (cn._hc) ? ((cn._io) ? 'user' : 'user') : 'user_orange') + '.png';
				this.strOutput += '" alt="" />';
