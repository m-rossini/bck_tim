...
<script language="Javascript">
 function closeWindow() {
  ...
  function submitForm() {
  	document.forms['plansForm'].submit();
  	return;
  }
</script>
...
<bean:define id="currentDiscount" name="<%=RequestScopeConstants.REQUEST_REQINFO_KEY%>" type="br.com.auster.tim.billcheckout.model.ProgressiveDiscountPlans"/>

<bean:define id="user" name="<%=SessionScopeConstants.SESSION_USERINFO_KEY%>" scope="session"  type="br.com.auster.security.model.User"/>

  <form name="plansForm" action="<html:rewrite page="/discountTablePlansPopup.do"/>" method="post" id="plansForm">
	...
	<input type="hidden" name="discountDescUid" value="<%=String.valueOf(currentDiscount.getDiscountUid())%>"/>
	<!-- Filter -->
...
<logic:notEmpty name="<%=RequestScopeConstants.REQUEST_LISTOFRESULTS_KEY%>" scope="request">
	...
	<logic:iterate name="planList" id="planInfo">
		...
		<tr height="10" class="text8">
      		<td width="15%"><input type="checkbox" name="planUid" value="<bean:write name="planInfo" property="uid" format="########"/>" <% if (currentDiscount.containsPlanUid( ((Plan)planInfo).getUid() )) { out.print(" checked "); } %>/></td>
...
<logic:empty name="<%=RequestScopeConstants.REQUEST_LISTOFRESULTS_KEY%>" scope="request">
		...
		<script language="Javascript">
	   	...
	   	<form name="moveToPage" action="<html:rewrite page="/discountTablePlansPopup.do"/>" method="post">
