...
<script language="javascript">
	...
	function openWindow(tariffZoneUid, serviceUid, rateTimeZoneUid, incomingFlag) {
		window.open("<html:rewrite page="/tariffsPopup.do"/>?operation=listHistVoice" +
					"&rowSelected=" + tariffZoneUid + '-' + serviceUid + '-' + rateTimeZoneUid + '-'
					+ incomingFlag, "tariffsPopup", "scrollbars,resizable,width=650,height=400");
	}
</script>
		    	...
		    	<table width="100%" align="center" class="table" cellpadding="3" cellspacing="0">
				  	<tr height="20" class="table-column-title">
				    	<td width="12%" class="table-title">
				    		<a href="javascript:submitForm('listVoice')" style="color:#FFF;">
			   			...
			   			<logic:notEmpty name="resultList">
							  ...
							  <display:column headerClass="table-column-title" title="Zona tarifária" maxLength="45">
							  	<a href="javascript:openWindow('<c:out value="${listTable.tariffZoneUid}"/>',
							  								   ...
							  								   '<c:out value="${listTable.rateTimeZoneUid}"/>',
							  								   '<c:out value="${listTable.incomingFlag}"/>')">
							  		<c:out value="${listTable.tariffZone}"/>
							  	</a>
							  </display:column>
							  <display:column headerClass="table-column-title" property="rateTimeZone" title="Mod. horária"/>
							  ...
							  <display:column headerClass="table-column-title" property="incomingFlag" title="In?"/>
							  <display:column headerClass="table-column-title" property="initVolume" title="Volume inicial"/>
							  <display:column headerClass="table-column-title" property="endVolume" title="Volume final" />
							  <display:column headerClass="table-column-title" property="stepVolume" title="Step"/>
							  <display:column headerClass="table-column-title" property="amount" title="Valor" format="R$ {0,number,#,##0.00##}"/>
