...
<script language="javascript">
	...
	function openWindow(serviceUid) {
		window.open("<html:rewrite page="/tariffsPopup.do"/>?operation=listHistService" +
					"&rowSelected=" + serviceUid, "tariffsPopup",
					"scrollbars,resizable,width=660,height=400");
	}
</script>
		    	...
		    	<table width="100%" align="center" class="table" cellpadding="3" cellspacing="0">
				  	<tr height="20" class="table-column-title">
				    	...
				    	<td width="12%" class="table-title">
			    			<a href="javascript:submitForm('listService')" style="color:#FFF;">
		    	...
		    	<td colspan="2" valign="top">
		   		<div id="search-results" class="text8b" style="overflow:auto;width:600px">
		   			...
		   			<logic:notEmpty name="resultList">
						  ...
						  <display:column headerClass="table-column-title" title="Serviço" maxLength="40">
						  	<a href="javascript:openWindow('<c:out value="${listTable.uid}"/>')">
							  	<c:out value="${listTable.description}"/>(<c:out value="${listTable.shortDesc}"/>)
						  	</a>
						  </display:column>
						  <display:column headerClass="table-column-title" property="planDescr"     title="Plano" 	      maxLength="15"/>
						  ...
						  <display:column headerClass="table-column-title" property="oneTime" 	    title="Adicional"     format="R$ {0,number,#,##0.00##}"/>
